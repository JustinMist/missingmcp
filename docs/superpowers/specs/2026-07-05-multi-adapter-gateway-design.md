...
| End state | Development moved to `github.com/JustinMist/missingmcp` (done 2026-07-05, fork-style); `garmin-mcp-gateway` frozen as the prod snapshot the VPS deploys from; both legacy repos archived at the end | Deliberate fork-style split: prod stays pinned while the gateway evolves. Bugfixes needed in prod before the domain flip must be cherry-picked to the `legacy` remote. |
