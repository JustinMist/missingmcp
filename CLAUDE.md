...
- **`report.py`** — daily per-connector user-stats Slack report, computed from the DB. Disabled unless `SLACK_WEBHOOK_URL` is set. (The *hourly* health digest is a separate **external** job — `scripts/hourly_digest.py` + `.github/workflows/hourly-digest.yml` — because the app can't read its own stdout logs.)
...
- **`workers.py`** — `WorkerManager`: per-account lazy spawn under a lock, `/healthz` poll, idle reaper, LRU cap; worker output pumped into the structured log.
