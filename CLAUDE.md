...
- **Never modify or import `garmin_mcp`.** Interact with it *only* as a black box via its documented CLI entrypoint (`garmin-mcp`) and env vars (`GARMIN_MCP_TRANSPORT`, `GARMIN_MCP_HOST`, `GARMIN_MCP_PORT`, `GARMINTOKENS`, `GARMIN_IS_CN`). No source edits, no importing its internal modules.
...
- **`account_key`** is adapter-owned and scoped by `adapter`. Garmin uses `cn:<normalized-email>` / `global:<normalized-email>` so the same email can exist in both domains; an existing pre-region Global row deliberately keeps its naked email key so old Bearers remain valid. Other adapters retain their existing identity rules. `(adapter, account_key)` is the join key across every table and `account_key` keys the worker registry. A Bearer token carries its `adapter`; the proxy rejects it on a different adapter's `/mcp`.
- **Secret handling:** the Garmin **password is never persisted or logged**; before MFA state enters `AuthState`, the continuation removes both garminconnect's outer password and the password-bearing prepared-request body retained by its widget flow. It also detaches garminconnect's request-private tokenstore path before that temporary directory is released, so an MFA-time refresh cannot recreate an unmanaged plaintext token file. **Bearer tokens and client secrets are stored only as SHA-256 hashes.** Garmin tokens and their account region live in a versioned adapter blob encrypted with AES-256-GCM; workers receive only a 0600 raw token file plus a 0600 region marker (`dirs 0700`). Garmin worker spawn removes inherited `GARMIN_EMAIL`, `GARMIN_PASSWORD`, both `_FILE` variants and `GARMINTOKENS_BASE64`, so stale account tokens fail instead of falling back to deployment-level credentials. Structured and third-party text logs redact quoted/dict/JSON secret assignments, escaped quoted values and complete Authorization fields, as well as secret-named fields. Logs carry at most an 8-char hash prefix. (A remote-strategy adapter may need to keep login credentials in its blob — the upstream authenticates every request — but they still live only inside the encrypted blob, never logged, never materialized to files.) **Telemetry egress obeys the same rule:** identity + metadata only, never MCP bodies, credentials or form contents — the account identity travels only as PostHog's `distinct_id`.
...
- **Garmin rotates too — inside the worker's token file.** garth rewrites `garmin_tokens.json` on Garmin's refresh-token rotation, so `WorkerManager` re-packs it with the unchanged region and conditionally persists it against the previous blob generation. Capture runs periodically, on reap/evict/shutdown, on respawn, and after a stopped startup process, always after exit is confirmed; a failed stopped-worker capture stays pending and writes a private generation-digest marker so it can be retried after restart. The marker is valid only while its expected generation still matches the DB; this check happens before parsing disk, so a verified browser re-login or account deletion invalidates even a torn/missing old capture. A fresh forward also seeds its read-only expected region from that DB blob before restart read-back—never by materializing over the candidate—so a missing/invalid/opposite sidecar cannot downgrade CN to Global. Under the account lock, the manager reloads the authoritative DB blob before deciding reuse/replacement and again after a possible CAS loss, so a queued request's stale blob is only a hint. Never persist a torn or structurally incomplete token object, a region-marker mismatch, or unknown-lineage disk state. The recovery script reports such historical drift as `untrusted-generation`; `--apply` only accepts manager-proven pending captures. Workdirs use a SHA-256 mapping of the full account key, never lossy character replacement. Legacy lossy directories containing a transformed character or `_` have unknowable historical ownership and must never be auto-backfilled, even if only one matching DB row remains.
- **Verify-then-persist:** in `oauth.py`, `adapter.verify` is the only "expectedly failing" step and gates `_finish` (which does upsert + code-mint + redirect) on **every** authorize path. Verification may rotate credentials, so its structured result's verified blob — not the candidate blob — is persisted. A login/verify failure re-renders the form; a wrong MFA code re-prompts. Don't move `adapter.verify` back into `_finish`.
...
- **Worker reap/evict/replace all gate on `inflight`.** `reap_idle`, `_enforce_cap`, and `ensure_worker`'s reuse/replace path must never kill a worker with `inflight>0`; `ensure_worker` holds the worker in-flight across its `_healthy()` await (so a concurrent reap can't pop a just-validated worker), and `_enforce_cap` counts `_reserved` spawns toward the cap. Async request/reap/evict paths hold the target account lock and yield with `asyncio.sleep` while waiting for exit; only synchronous process shutdown uses the blocking helper. Every stop path sends SIGTERM, waits, escalates to SIGKILL, then confirms exit before reading or rematerializing credentials. An unregistered startup process is recorded under its account before any cancellable cleanup await, so repeated cancellation cannot release its workdir to a newer login while it may still write. **A terminated worker's port cools down** (skipped by `_alloc_port`, which round-robins instead of lowest-free-first) until its process is observed dead — with no unsafe hard expiry. An unconfirmed startup process also blocks reuse of its account workdir. A dying uvicorn can still answer `/healthz`, and validating a fresh spawn against its predecessor's listener hands the forward a dead port (~100 user-facing 502s/day before the fix; reliability ticket 12). The proxy additionally retries a `ConnectError`ed worker forward once, re-running `ensure_worker`.
...
- **The adapter owns identity normalization:** Garmin first normalizes the email via `base.normalize_account_key`, then adds its region identity (subject to the legacy-Global exception above). `oauth._finish` persists `LoginOk.account_key` as-is.
...
- Live Garmin network calls are mocked, but dependency-contract tests run against the exact installed `garminconnect==0.3.6`: its real region-aware DI endpoint selection, token refresh/dump and widget-MFA continuation state are exercised without contacting Garmin. Docker and CI assert that the pinned worker venv loads that same reviewed version after its own frozen sync. The worker manager and proxy are tested against a **fake worker HTTP server** (`tests/conftest.py::fake_worker`); the remote strategy against a **fake remote upstream** (`fake_remote`) driven through `conftest.StubRemoteAdapter` (`tests/test_remote_forward.py` + the generic authorize-flow tests in `test_oauth.py`); the local strategy the same way, through `conftest.StubLocalAdapter` (`tests/test_local_forward.py`); the upstream-OAuth login shape generically through `conftest.StubUpstreamOAuthAdapter` (`test_oauth.py`); backups against the same fake upstream posing as S3 (`tests/test_backup.py` — the SigV4 signer was additionally verified once against a real bucket). The whoop adapter itself (both pieces wired together) is covered end-to-end against a **fake WHOOP upstream** (`tests/conftest.py::fake_whoop`, a `FakeWhoopUpstream`) in `tests/test_whoop_e2e.py`.
- Consequently the **real Garmin/WHOOP network login and refresh paths are not covered by automated tests**. A manual end-to-end smoke test — Garmin (email/password, MFA) and WHOOP (provider sign-in, tool calls, and a token refresh once the access token expires) — is the release gate before connecting real users.
